...
export default function LandingPage() {
  ...
  return <div className="min-h-screen bg-gradient-to-b from-background to-secondary">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-4 py-20">
        <img src={logoAprisco} alt="APRISCO" className="h-48 md:h-64 mb-16 animate-fade-in" />
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-center mb-6 animate-fade-in-up">
          
        </h1>
        <p className="text-lg md:text-xl lg:text-2xl text-muted-foreground max-w-3xl text-center mb-10 animate-fade-in-up px-4" style={{
        animationDelay: "100ms"
      }}>
          Gerencie suas igrejas, membros e visitantes com eficiência e organização em um único lugar
        </p>
        <Button size="lg" className="btn-hover-lift shadow-glow text-lg px-8 py-6 animate-scale-in" style={{
      ...
      <section className="py-20 px-4 bg-background/50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {features.map((feature, index) => <Card key={feature.title} className="stat-card hover-scale border-2 transition-all duration-300 hover:border-primary/20 animate-fade-in-up" style={{
      ...
      <section className="py-20 px-4 bg-primary/5">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <Button size="lg" className="btn-hover-lift shadow-glow text-lg px-8 py-6" onClick={() => navigate("/auth")}>
            Começar Agora
            <ArrowRight className="ml-2 h-5 w-5" />
          </Button>
        </div>
